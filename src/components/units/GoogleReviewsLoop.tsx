...
export default function GoogleReviewsLoop({ reviews, googleUrl }: GoogleReviewsLoopProps) {
  ...
  const normalizedIndex = activeIndex % safeReviews.length;
  const activeReview = safeReviews[normalizedIndex];
  ...
  return (
    <div className="mt-5 overflow-hidden rounded-[1.5rem] border border-red-600/15 bg-black/25 p-4 shadow-[0_0_24px_rgba(220,38,38,0.08)]">
        ...
        <div className="mt-4 flex flex-wrap gap-2" aria-label="Navegação das avaliações">
          {safeReviews.map((review, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${normalizedIndex === index ? 'w-7 bg-red-500' : 'w-2 bg-white/20 hover:bg-white/40'}`}
            />
          ))}
        </div>
      )}
      ...
      {googleUrl && (
        <a href={googleUrl} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex rounded-full border border-white/10 bg-white/5 px-4 py-2.5 text-[0.68rem] font-black uppercase tracking-[0.14em] text-slate-200 transition hover:border-red-600 hover:text-red-300">
