...
function Stars({ rating }: { rating: number }) {
  ...
  return (
    <div className="flex gap-1 text-red-400" aria-label={`${roundedRating} estrelas`}>
...
export default function GoogleReviewsLoop({ reviews, googleUrl }: GoogleReviewsLoopProps) {
  const safeReviews = useMemo(
    () => reviews.filter((review) => review.rating >= 4 && review.text.trim()).slice(0, 5),
    [reviews]
  );
  ...
  if (!safeReviews.length) {
    return (
      <div className="mt-7 rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-5 text-sm leading-relaxed text-slate-400">
        Comentarios positivos do Google serao exibidos aqui quando a integracao retornar avaliacoes com texto.
      </div>
    );
  }
        ...
        <div className="mt-5 flex gap-2">
          {safeReviews.map((review, index) => (
            <button
              ...
              type="button"
              aria-label={`Ver avaliacao ${index + 1}`}
